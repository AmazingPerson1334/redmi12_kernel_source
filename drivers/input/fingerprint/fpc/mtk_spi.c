...
static const char * const pctl_names[] = {
	//"fpsensor_fpc_power_on",
	//"fpsensor_fpc_power_off",
	"fpsensor_fpc_rst_low",
	"fpsensor_fpc_rst_high",
	"fpsensor_fpc_cs_high",
	"fpsensor_fpc_cs_low",
};
...
{
	int rc = 0;
        pr_err("%s enter, clocks_enabled = %d\n", __func__,fpc->clocks_enabled);
	if(fpc->clocks_enabled == enable)
		return rc;
	if (enable) {
                printk("%s  mt_spi_enable_master_clk\n", __func__);
		mt_spi_enable_master_clk(fpc->spidev);
		fpc->clocks_enabled = true;
		rc = 1;
	} else {
                printk("%s mt_spi_disable_master_clk\n", __func__);
...
static int fpc_read_id(struct fpc_data *fpc,struct spi_device *spidev)
 {
     int status;
     const int len = 3;
	...
	mt_spi_enable_master_clk(spidev);
    msleep(2);
...
{
	pr_info("[XMFP]: %s: fpc fp unblank start\n", __func__);
	mdss_prim_panel_fb_unblank(FP_UNLOCK_REJECTION_TIMEOUT);
	pr_info("[XMFP]: %s: fpc fp unblank end\n", __func__);
}
...
*/
	...
	** since this is interrupt context (other thread...) */
	...
	pr_info("[XMFP] %s fpc->wait_finger_down:%d  fpc->fb_black:%d\n",__func__,fpc->wait_finger_down,fpc->fb_black);
				 ...
				 unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && event == FB_EVENT_BLANK) {
		...
		case FB_BLANK_POWERDOWN:
			printk("[XMFP] %s lcd off notify\n", __func__);
			fpc->fb_black = true;
			break;
		case FB_BLANK_UNBLANK:
			printk("[XMFP] %s lcd on notify\n", __func__);
			fpc->fb_black = false;
			break;
		default:
...
static int mtk6797_probe(struct spi_device *spidev)
{
    ...
    //select_pin_ctl(fpc, "fpsensor_fpc_power_on");
        select_pin_ctl(fpc, "fpsensor_fpc_cs_low");
	mdelay(1);
	fpc_regulater_enable(dev, 1);
	usleep_range(FPC_RESET_HIGH1_US, FPC_RESET_HIGH1_US + 100);
        select_pin_ctl(fpc, "fpsensor_fpc_cs_high");
	(void)hw_reset(fpc);
	dev_err(dev, "%s clocks_enabled = %d\n", __func__,fpc->clocks_enabled);
        fpc->clocks_enabled = false;
	if ( 0 != fpc_read_id(fpc,spidev))
        {
		select_pin_ctl(fpc, "fpsensor_fpc_rst_low");
		select_pin_ctl(fpc, "fpsensor_fpc_cs_low");
		//select_pin_ctl(fpc, "fpsensor_fpc_power_off");
		fpc_regulater_enable(dev, 0);
        ...
        dev_err(dev, "%s, clocks_enabled = %d\n", __func__,fpc->clocks_enabled);
	//select_pin_ctl(fpc, "fpsensor_fpc_power_off");
	node_eint = of_find_compatible_node(NULL, NULL, "mediatek,goodix-fp");
	if (node_eint == NULL) {
		rc = -EINVAL;
	...
	fb_register_client(&fpc->fb_notifier);
...
static int mtk6797_remove(struct spi_device *spidev)
{
	...
	fb_unregister_client(&fpc->fb_notifier);
